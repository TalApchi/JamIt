...
import { DistortionGuitarInstrument } from "../components/DistortionGuitar/DistortionGuitarInstrument";
...
export function DistortionGuitarScreen({
  rootNote,
  mode,
  ...
  // The Distortion Guitar background image is landscape (1536x1024); the
  // rest of the app is portrait, so this screen locks to landscape on mount
  // and restores portrait on unmount instead of relying on the phone's
  // physical rotation.
  useEffect(() => {
    ScreenOrientation.lockAsync(ScreenOrientation.OrientationLock.LANDSCAPE).catch((error) => {
      console.warn("Unable to lock Distortion Guitar screen to landscape", error);
    });
