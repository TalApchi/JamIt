...
import { SynthBassInstrument } from "../components/SynthBass/SynthBassInstrument";
...
export function SynthBassScreen({
  rootNote,
  mode,
  ...
  // The Synth Bass background image is landscape (1536x1024); the rest of
  // the app is portrait, so this screen locks to landscape on mount and
  ...
  useEffect(() => {
    ScreenOrientation.lockAsync(ScreenOrientation.OrientationLock.LANDSCAPE).catch((error) => {
      console.warn("Unable to lock Synth Bass screen to landscape", error);
    });
