...
//  - 7 notes (C#4, F4, A4, C#5, F5, A5, C6) are the pack's own real
//    recorded takes, used directly from `assets/audio/DistortionGuitar/`.
//  - 28 notes have no matching recording and are pre-rendered offline into
//    `generated/` by scripts/generate-guitar-shifted-samples.js, by
//    resampling the nearest recorded anchor (Lanczos kernel, same technique
//    as every other instrument's own generated/ files). The highest anchor
//    (C6, midi 84) is 10 semitones below the top of the range (94) -- the
//    top 11 notes (84..94) all shift up from C6, the largest shift any
//    instrument in this app has needed; verified within +-5.4 cents (see
...
//    because runtime playbackRate-based pitch shifting was already
//    confirmed UNRELIABLE on-device for the Melodica/Piano -- the same
//    underlying expo-audio/OS behavior applies to every instrument in this
//    app. Every note therefore plays at playbackRate 1.0; the
//    nearest-sample + rate = 2^(shift/12) resolver only ever engages for
//    targets outside 60..94, which the scale engine (at either octave row)
//    can never produce.
...
export const DISTORTION_GUITAR_SAMPLE_DEFS: DistortionGuitarSampleDef[] = [
  ...
  { filename: "C#4.wav", noteWithOctave: "C#4", midi: 61, gain: 1.0 },
  ...
  { filename: "F4.wav", noteWithOctave: "F4", midi: 65, gain: 1.0 },
  ...
  { filename: "A4.wav", noteWithOctave: "A4", midi: 69, gain: 1.0 },
  ...
  { filename: "C#5.wav", noteWithOctave: "C#5", midi: 73, gain: 1.0 },
  ...
  { filename: "F5.wav", noteWithOctave: "F5", midi: 77, gain: 1.0 },
  ...
  { filename: "A5.wav", noteWithOctave: "A5", midi: 81, gain: 1.0 },
  ...
  { filename: "C6.wav", noteWithOctave: "C6", midi: 84, gain: 1.0 },
